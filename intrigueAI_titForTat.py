...
from player import Application, ApplicationLog, ConflictLog, EarningsLog, PlacementLog, Player
from square import Square


class TitForTat(IntrigueAI):
    """TitForTat AI maintains local memory of profits gained and lost as a result of player actions 
    rewarding the best ratios and punishing the worst."""
    ...
    #         #TODO: Update
    #         ordered_conflict_ascending = sorted(conflict, key=lambda a : self.gain_loss_model[a[0].owner])
    #         highest_value_app = ordered_conflict_ascending[-1]
    #         file.write("Selected "+repr(highest_value_app)+ "because I like them most. "+repr(self.gain_loss_model)+"\n")
    #         conflict_log.append( (conflict,highest_value_app) )
    ...
    #     self.place_according_to_like(state, internal_placements, applications_to_place, empty_squares, placement_log,file)
    ...
    # def place_according_to_like(self, state, internal_placements, applications_to_place:list[Application], empty_squares:list[Square], placement_log:PlacementLog,file:TextIO):
    #     """Find a square to place the applications according to their bribes.
    #     Updates gain/loss model according to good or bad bribe offers."""
        
    #     # preference_order_ascending = [c for c,i in sorted(list(self.gain_loss_model.items()),key=lambda t: t[1])]
    ...
    #     #Check applications in order of preference.
    #     applications_to_place = sorted(applications_to_place, key=lambda a : self.gain_loss_model[a[0].owner])
    ...
    #         #Update model according to offer value. TODO: Place this in separate update function.
    #         self.gain_loss_model[piece.owner] += bribe - square.value

    #         if square in empty_squares:
    #             #Good opinion, place in requested location.
    #             if self.gain_loss_model[piece.owner] >= square.value:
    #                 placement_log.append( (app, square) )
    #                 empty_squares.remove(square)
    #                 file.write("Has profit equivalent to square, accepted.")
    #                 continue
    #         else:
